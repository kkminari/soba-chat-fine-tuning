...
- [ ] **최종 결정: ___ B** (사유: Phase 6 정성 평가 결과 대기)
- [ ] `docs/results/experiment2_8b_baseline_report.pdf` 리포트 작성
...
- [ ] `configs/training_config_final.yaml` 생성
- [ ] `docs/results/experiment3_optimization_report.pdf` 리포트 작성
...
#### 4-3. 수동 평가 (블라인드 테스트)
- [ ] 30건 랜덤 샘플링 완료
- [ ] 평가 시트 준비 (Google Sheets 또는 CSV)

| # | 평가 항목 | PASS 기준 | 결과 | PASS/FAIL |
|---|----------|----------|------|-----------|
| 1 | [ ] 코멘트 톤 적절성 | >= 85% | ___/30 (___%) | |
| 2 | [ ] 질문 리프레이징 자연스러움 | >= 80% | ___/30 (___%) | |
| 3 | [ ] Claude Sonnet 대비 품질 (A/B 20건) | 동등 이상 | FT 승 ___건 / 무승부 ___건 / Sonnet 승 ___건 | |

#### 4-4. 통합 세션 테스트 (end-to-end)

| # | 시나리오 | 건수 | 확인 항목 | 결과 |
|---|---------|------|----------|------|
| 1 | [ ] 정상 플로우 | 10건 | 8질문 완주, JSON 100%, 톤 일관성 | ___/10 통과 |
| 2 | [ ] 짧은 응답 (1~3단어) | 5건 | 코멘트 자연스러움 | ___/5 통과 |
| 3 | [ ] 부정적/무관심 응답 | 5건 | 공감 톤 유지, 비강요 | ___/5 통과 |
| 4 | [ ] 긴 응답 (100자+) | 5건 | 핵심 포착, 코멘트 적정 길이 | ___/5 통과 |
| 5 | [ ] 무관한 응답 | 3건 | retry 메시지 적절 생성 | ___/3 통과 |
...
- [ ] 통합 세션 테스트 전체 PASS
- [ ] FAIL 항목 대응 완료 (해당 시)
- [ ] `docs/results/phase6_evaluation_report.pdf` 리포트 작성
- [ ] **Phase 6 최종 판정: `[ ]` GO / `[ ]` NO-GO**
  - NO-GO 시 대응: ___
...
- [ ] `docs/results/experiment2_7b_baseline_report.pdf`
- [ ] `docs/results/experiment3_optimization_report.pdf`
- [ ] `docs/results/phase6_evaluation_report.pdf`
- [ ] `configs/training_config_final.yaml`
- [ ] 최종 어댑터 `src/outputs/adapter/` 저장 완료
